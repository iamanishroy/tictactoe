    ...
    .on("value", function (snapshot) {
      if (snapshot.val()) {
        ...
        if (!Object.keys(snap).includes("close")) {
          if (
            Object.keys(snap).includes("hostConcent") &&
            Object.keys(snap).includes("playerConcent")
          ) {
          ...
          if (snap.current == snap.host) {
            $("#players")
              .html(`<input id="tab0" type="radio" name="tabs" checked><label for="tab0">${
              snap.host
            } ${
              hostPoint > 0
                ? '<span class="badge danger">' + hostPoint + "</span>"
                : ""
            }</label>
                <input id="tab1" type="radio" name="tabs"><label for="tab1">${
                  snap.player
                } ${
              playerPoint > 0
                ? '<span class="badge danger">' + playerPoint + "</span>"
                : ""
            }</label>`);
          } else {
            $("#players")
              .html(`<input id="tab0" type="radio" name="tabs"><label for="tab0">${
              snap.host
            } ${
              hostPoint > 0
                ? '<span class="badge danger">' + hostPoint + "</span>"
                ...
                <input id="tab1" type="radio" name="tabs" checked><label for="tab1">${
                  snap.player
                } ${
              playerPoint > 0
                ? '<span class="badge danger">' + playerPoint + "</span>"
          ...
          if (declareOnce && win != "N") {
            ...
            pop(
              "Yo..Ho..!!",
              "",
              `${win == "X" ? snap.player : snap.host} won the Match!!`
            );
            if (name == snap.host) {
              winner.push(win);
              firebase
